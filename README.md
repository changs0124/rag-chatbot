...
>
> **도커 이미지 빌드만 예외로 `mvnw` 를 쓰지 않는다.** wrapper 는 빌드마다 Maven 배포판을 Maven Central 에서 내려받는데, 그 요청이 거절되면 이미지 빌드가 통째로 멎기 때문이다(실제로 있었던 일이다). 대신 같은 버전이 들어 있는 `maven:3.9.16-eclipse-temurin-17-alpine` 을 쓴다 — 버전을 맞춰 두었으므로 로컬·CI 와 갈리지 않는다.

## 핵심 요구 (요약)

- 입력 : 이미지 첨부(사진 선택 · 카메라 촬영 · 드래그앤드롭 · 붙여넣기) + 텍스트 질문
  (**이미지가 아닌 파일은 받지 않는다** - 2026-07-28 에 범위에서 빼고 2026-08-18 에 화면 경로까지 지웠다.
  정본은 `docs/01_specs/requirements.md` 의 범위 표다)
