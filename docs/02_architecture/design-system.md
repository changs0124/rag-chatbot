...
「자료 없음」 배지와 관리자 오류 배너를 위해 생겼다 — 쓰이지 않는 토큰은 다음 사람이 아무 데나
갖다 쓰는 근거가 된다. #155 에서 각각 한 자리씩 늘어 지금은 **둘 다 두 곳**이다 :
`highlight` 는 첨부 실패 아이콘, `danger-soft` 는 `MyPage` 오류 배너(관리자 쪽과 같은 모양인데
#151 이 한쪽만 고쳤던 자리다).
