...
- **SSE 타임아웃과 업스트림 읽기 타임아웃이 둘 다 10분이다.** `app.chat.sse-timeout-ms`(기본 600000)와
  `OpenAiRealService` 의 `setReadTimeout` 이 같은 값이라, 타임아웃이 났을 때 어느 갈래로 떨어질지가
  밀리초 경합으로 갈린다. 저장에는 `timed_out` 으로 남아 사후에 구분되지만(#84), **갈래 자체를
  결정적으로 만들려면 읽기 타임아웃을 더 짧게 두어야 한다** — 워커 스레드·동시성 권한 점유와 같은
  원인이라 그쪽에서 함께 다룬다.
