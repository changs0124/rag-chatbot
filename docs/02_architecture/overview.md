...
> 코드베이스 스캔 기준으로 작성한 초안 (2026-07-29). 구조가 바뀌면 이 문서를 함께 갱신한다.
...
| `/admin` | 문서 관리 · 사용자 관리 | **미구현** (백엔드 API 만 있음) |
| 그 외 | `/`로 리다이렉트 | 해당 없음 |
...
- **관리자 화면이 아직 없다.** `/api/admin/**` 은 동작하지만 `/admin` 페이지가 없어 지금은 API 로만 쓸 수 있다.
