...
- **Spring Boot 3.5 는 OSS 지원이 끝났다**(2026-06-30). 물고 있는 `3.5.16` 이 마지막 무상 패치이고
  `3.5.17` 은 없다. 새 CVE 가 공개되면 올릴 버전이 Maven Central 에 없어 `deps` CI 게이트가 막힌다 —
  **막히는 시점을 우리가 고를 수 없다.** 권장 경로는 4.0 이 아니라 **4.1** 이다(4.0 은 2026-12 종료).
  `docs/04_tasks/backlog.md` 「높은 우선순위」 참고.
