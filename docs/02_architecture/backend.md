...
- **오류 상태 코드를 컨트롤러가 만들지 않는다.** 서비스가 도메인 예외를 던지고 `GlobalExceptionHandler` 가
  ...
  `CorsFilter` 가 평문 `Invalid CORS request`). 프론트는 두 경우 `요청 실패 (n)` 로 떨어진다
...
- **성공 상태는 컨트롤러가 정한다** — 문서 업로드가 `@ResponseStatus(CREATED)`(201), 삭제 3곳이
  ...
  그 밖의 핸들러는 DTO 를 그대로 돌려주고 상태를 지정하지 않는다. 채팅만 `SseEmitter`(text/event-stream) 다.
...
- 승격·강등은 환경변수 `ADMIN_EMAILS` 명단으로만 일어난다. **앱에 권한 상승 API 가 없다.**
  ...
  대안이었던 **첫 가입자 자동 승격**은 DB 초기화 시점에 외부인이 먼저 가입하면 그가 관리자가 되어
  기각했고, **DB 수동 UPDATE** 는 배포마다 DB 접속이 필요해 절차가 문서에만 존재하게 되므로 기각했다.
  대가는 **명단 변경에 재기동이 필요**하다는 것이다 — 사내 소규모에서 관리자 교체는 드물어 감수한다.
- **권한 없음도 404 다.** 관리 기능의 존재 자체를 드러내지 않는다(`AdminAccessGuard`).
- **가입은 허용 도메인으로만 받는다**(`ALLOWED_EMAIL_DOMAINS`, FEAT-AUTH-001). `live` 에서 명단이
  비면 기동을 막는다 — "비면 전원 허용"으로 두면 배포에서 변수를 빠뜨렸을 때 조용히 가입이 열린다.
  **도메인 검사는 중복 검사보다 먼저** 한다. 순서가 뒤집히면 거절할 주소에 "이미 가입된 이메일"을
  돌려주게 되어 계정 존재 여부가 샌다. 서브도메인은 포함하지 않으며(정확히 일치),
  `ADMIN_EMAILS` 는 검사를 면제받는다 — 면제가 없으면 관리자 부트스트랩이 막힌다.
  **검사는 가입 시점에만** 한다. 로그인에서도 막으면 정책 이전에 만들어진 계정이 통째로 잠긴다.
