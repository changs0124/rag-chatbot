...
**단 미인증 401 은** `GlobalExceptionHandler` 를 거치지 않고 `SecurityConfig` 의 진입점이 직접 내므로
Spring 기본 오류 본문이고 `message` 가 없다.
...
|------|------|--------------|
| 200 | 성공 | |
| 201 | 생성 성공 | 문서 업로드 한 곳뿐. 회원가입·대화 생성·첨부 업로드를 포함한 나머지 생성 경로는 전부 **200** 이다 |
| 204 | 본문 없음 | 삭제류 |
| 400 | 잘못된 요청 | |
| 401 | 인증 실패 | 토큰 없음·만료·무효 |
| **403** | — | **애플리케이션은 쓰지 않는다.** CORS 허용 목록에 걸린 요청만 Spring 이 낸다 |
