...
/**
 * 계정 도메인 화이트리스트 HTTP 계약 (FEAT-AUTH-001 · TC-AUTH-001 · 002 · 008 · 011).
 *
 * <p>판정 자체는 {@code EmailDomainPolicyTest} 가 단위로 본다. 여기서는 <b>상태 코드와 검사 순서</b>,
 * 그리고 거절이 DB 에 흔적을 남기지 않는지를 본다.
 *
 * <p>뿌리 관리자({@code root@rag.test})가 허용 도메인 밖인데도 기동이 되는 이유는 정책이
 * <b>관리자 명단을 면제</b>하기 때문이다. 면제가 없으면 첫 관리자를 못 만들어 이 클래스 전체가 뜨지 않는다.
 */
...
class AuthDomainAllowlistTest extends AbstractPgIntegrationTest {
	...
	private ResponseEntity<Map> issue(String email) {
		return issueRaw(email, "사용자");
	}
	...
	/** TC-AUTH-001 : 허용 도메인은 계정이 발급된다 */
	@SuppressWarnings("unchecked")
	@Test
	void allowed_domain_can_be_issued() {
		var res = issue("hong@company.com");
	...
	void outside_domain_is_rejected_and_leaves_no_row() {
		var res = issue("someone@gmail.com");
	...
	void domain_match_is_case_insensitive() {
		var res = issue("Hong.Case@Company.COM");
	 ...
	 *
	 * <p>순서가 뒤집히면 거절할 주소에 대해 409(이미 있음)를 돌려주게 되어 <b>계정 존재 여부가 샌다.</b>
	...
	void domain_check_runs_before_duplicate_check() {
		...
		var res = issue("legacy@gmail.com");
	...
	void duplicate_inside_allowed_domain_is_still_conflict() {
		issue("dup@company.com");

		var second = issue("dup@company.com");
	 ...
	 *
	 * <p>검사는 발급 시점에만 한다. 로그인에서도 막으면 정책 도입 이전에 정당하게 만들어진 계정이
